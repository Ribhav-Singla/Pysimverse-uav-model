...
async function runTrainingScript() {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`💰 Training ${PPO_TYPE.toUpperCase()} PPO (Augmented Rewards)`);
...
async function uploadWeights() {
    try {
        ...
        // Filter for AR PPO weight files
        const arFiles = files.filter(file => file.toLowerCase().includes('ar'));
...
async function uploadTrainingArtifacts() {
    try {
        ...
        // Define training artifacts to upload for AR PPO
        const artifacts = [
