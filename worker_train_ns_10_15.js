...
async function downloadWeights() {
    try {
        console.log(`📥 Downloading ${PPO_TYPE.toUpperCase()} PPO weights from previous stage...`);
        const weightsDir = path.join('PPO_preTrained', 'UAVEnv');
        
        // Create directory if it doesn't exist
        await fs.mkdir(weightsDir, { recursive: true });

        // Download NS PPO weights from stage 2
        const weightFile = 'NS_PPO_UAV_Weights.pth';
        ...
        try {
            ...
            return { downloadCount: 1, errorCount: 0 };
        } catch (err) {
            console.error(`❌ Failed to download ${weightFile}:`, err.message);
            throw new Error(`Failed to download weights from previous stage: ${err.message}`);
        }
    } catch (err) {
...
async function runTrainingScript() {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`🧠 Training ${PPO_TYPE.toUpperCase()} PPO - ${STAGE_NAME}`);
    ...
    const result = await new Promise((resolve, reject) => {
        ...
        function tryNextCommand() {
            ...
            const child = exec(`${cmd} -u training.py --ppo_type ${PPO_TYPE} --start_level ${START_LEVEL} --end_level ${END_LEVEL}`, {
            ...
            child.on('close', (code, signal) => {
                ...
                if (code === 0) {
                    console.log(`\n✅ ${PPO_TYPE.toUpperCase()} PPO training (${STAGE_NAME}) completed successfully after ${elapsed} minutes`);
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`✅ ${PPO_TYPE.toUpperCase()} PPO training (${STAGE_NAME}) completed`);
    console.log(`${'='.repeat(60)}\n`);
...
async function uploadWeights() {
    try {
        console.log(`🔄 Uploading ${PPO_TYPE.toUpperCase()} PPO weights (${STAGE_NAME}) to Cloudflare R2...`);
        const weightsDir = path.join('PPO_preTrained', 'UAVEnv');
        let uploadCount = 0;
        let errorCount = 0;
        ...
        // Filter for NS PPO weight files
        const nsFiles = files.filter(file => file.toLowerCase().includes('ns'));
...
async function uploadTrainingArtifacts() {
    try {
        console.log(`🔄 Uploading ${PPO_TYPE.toUpperCase()} PPO training plots and logs (${STAGE_NAME}) to Cloudflare R2...`);
        let uploadCount = 0;
        let errorCount = 0;

        // Define training artifacts to upload for NS PPO (final stage)
        const artifacts = [
        ...
        for (const artifact of artifacts) {
            try {
                ...
                // Final stage uploads to the main TrainingArtifacts folder
...
async function main() {
    console.log(`🚀 Starting ${PPO_TYPE.toUpperCase()} PPO Training (${STAGE_NAME} - FINAL) and Weight Upload Process...`);
    console.log(`⏰ Timestamp: ${new Date().toISOString()}\n`);

    try {
        // Step 1: Download weights from previous stage
        console.log('📥 Step 1: Downloading weights from previous stage...');
        ...
        // Step 2: Run training script for levels 10-15
        console.log(`🎓 Step 2: Running ${PPO_TYPE.toUpperCase()} PPO training (${STAGE_NAME})...`);
        await runTrainingScript();
        console.log(`✅ ${PPO_TYPE.toUpperCase()} PPO training (${STAGE_NAME}) completed successfully!\n`);

        // Step 3: Upload final trained weights
        console.log('📦 Step 3: Uploading final trained weights...');
        const weightsUploadResult = await uploadWeights();
        console.log(`✅ Final weights uploaded: ${weightsUploadResult.uploadCount} files\n`);

        // Step 4: Upload training plots and logs
        console.log('📊 Step 4: Uploading training plots and logs...');
        ...
        console.log(`\n✅ ${PPO_TYPE.toUpperCase()} PPO training (${STAGE_NAME} - FINAL) and upload process completed successfully!`);
        console.log(`🎉 All NS PPO training stages complete!`);
        process.exit(0);
