...
// Map PPO type to model name
const PPO_MODEL_MAP = {
    'vanilla': { name: 'Pure_Neural', file: 'Vanilla_PPO_UAV_Weights.pth' },
    'ar': { name: 'AR_PPO', file: 'AR_PPO_UAV_Weights.pth' },
    'ns': { name: 'Neurosymbolic', file: 'NS_PPO_UAV_Weights.pth' }
};
...
async function downloadWeights(ppoType) {
    try {
        console.log(`📥 Downloading ${ppoType.toUpperCase()} trained weights from Cloudflare R2...`);
        const weightsDir = path.join('PPO_preTrained', 'UAVEnv');
        
        // Create directory if it doesn't exist
        await fs.mkdir(weightsDir, { recursive: true });

        const weightFile = PPO_MODEL_MAP[ppoType].file;
        let downloadCount = 0;
        let errorCount = 0;

        try {
            const key = `PPO_preTrained/UAVEnv/${weightFile}`;
            console.log(`📥 Downloading ${weightFile}...`);
            
            const fileContent = await downloadFromR2(key);
            const localPath = path.join(weightsDir, weightFile);
            
            await fs.writeFile(localPath, fileContent);
            console.log(`✅ Downloaded ${weightFile}`);
            downloadCount++;
        } catch (err) {
            console.error(`❌ Failed to download ${weightFile}:`, err.message);
            errorCount++;
        }
        ...
        if (downloadCount === 0) {
            throw new Error(`No weights were downloaded successfully for ${ppoType}. Training job may have failed or weights were not uploaded to R2.`);
        }
...
async function executePythonScript(ppoType) {
    return new Promise((resolve, reject) => {
        ...
        function tryNextCommand() {
            ...
            // Run test with specific PPO type
            const child = exec(`${cmd} -u uav_test_single.py --ppo_type ${ppoType}`, {
            ...
            child.on('close', (code, signal) => {
                ...
                if (code === 0) {
                    console.log(`\n✅ ${ppoType.toUpperCase()} test completed successfully after ${elapsed} minutes`);
                    ...
                    console.error(`\n❌ Test terminated after ${elapsed} minutes`);
                    console.error(`   Exit code: ${code}`);
                    console.error(`   Signal: ${signal || 'none'}`);
                    
                    if (code === null) {
                        reject(new Error(`Test was killed after ${elapsed} minutes. Signal: ${signal || 'none'}`));
                    } else {
                        reject(new Error(`Test failed with exit code ${code} after ${elapsed} minutes`));
                    }
                }
            });
...
async function collectAgentData(ppoType) {
    try {
        const agentsDir = 'Agents';
        const agentName = PPO_MODEL_MAP[ppoType].name;
        const data = {
            agent: {},
            summary: null
        };
        ...
        // Read specific agent's data
        const agentPath = path.join(agentsDir, agentName);
        
        try {
            const stat = await fs.stat(agentPath);
            if (!stat.isDirectory()) {
                console.log(`⚠️ ${agentName} is not a directory`);
                return data;
            }
        } catch (err) {
            console.log(`⚠️ Agent folder not found: ${agentName}`);
            return data;
        }
        ...
        for (const obstacleFolder of obstacleFolders) {
            ...
            data.agent[agentName][obstacleFolder] = {
                maps: {}
            };
            ...
            for (const mapFolder of mapFolders) {
                ...
                data.agent[agentName][obstacleFolder].maps[mapFolder] = {
                    map_xml: null,
                    map_metadata: null,
                    trajectories: {}
                };
                ...
                try {
                    ...
                    data.agent[agentName][obstacleFolder].maps[mapFolder].map_xml = mapXMLContent;
                } catch (err) {
                ...
                try {
                    ...
                    data.agent[agentName][obstacleFolder].maps[mapFolder].map_metadata = JSON.parse(metadataContent);
                } catch (err) {
                ...
                try {
                    ...
                    for (const trajFile of trajectoryFiles) {
                        ...
                        data.agent[agentName][obstacleFolder].maps[mapFolder].trajectories[trajFile] = JSON.parse(trajContent);
                    }
                } catch (err) {
        ...
        console.error('❌ Error collecting agent data:', err);
        throw err;
    }
}

async function saveAgentData(data, ppoType) {
    try {
        const agentName = PPO_MODEL_MAP[ppoType].name;
        console.log(`🔄 Uploading ${agentName} data to Cloudflare R2...`);
        let uploadCount = 0;
        let errorCount = 0;
        ...
        if (data.summary) {
            try {
                const key = `Agents/${agentName}/results_summary.csv`;
                await uploadToR2(key, data.summary, 'text/csv');
                console.log(`✅ Uploaded results_summary.csv for ${agentName}`);
                uploadCount++;
            } catch (err) {
        ...
        if (data.agent[agentName]) {
            ...
            for (const [obstacleFolder, obstacleData] of Object.entries(data.agent[agentName])) {
        ...
        console.error('❌ Error uploading agent data to Cloudflare R2:', err);
        throw err;
    }
}

async function main() {
    console.log(`🚀 Starting UAV Testing for ${PPO_TYPE.toUpperCase()} PPO...`);
    console.log(`⏰ Timestamp: ${new Date().toISOString()}\n`);

    try {
        // Step 1: Download trained weights from R2
        console.log(`📥 Step 1: Downloading ${PPO_TYPE.toUpperCase()} trained weights...`);
        const weightsDownloadResult = await downloadWeights(PPO_TYPE);
        console.log(`✅ Weights downloaded: ${weightsDownloadResult.downloadCount} files\n`);

        // Step 2: Execute the UAV test for this PPO type
        console.log(`🚁 Step 2: Starting ${PPO_TYPE.toUpperCase()} UAV test...`);
        const { pythonCmd } = await executePythonScript(PPO_TYPE);
        console.log(`✅ ${PPO_TYPE.toUpperCase()} UAV test completed!\n`);
        ...
        // Step 4: Collect generated data for this agent
        console.log(`📊 Step 4: Collecting ${PPO_TYPE.toUpperCase()} generated data...`);
        const agentData = await collectAgentData(PPO_TYPE);

        console.log(`📦 Data collected for: ${PPO_MODEL_MAP[PPO_TYPE].name}`);

        // Step 5: Upload to Cloudflare R2
        console.log(`☁️  Step 5: Uploading ${PPO_TYPE.toUpperCase()} test results...`);
        const uploadResult = await saveAgentData(agentData, PPO_TYPE);

        console.log(`\n✅ ${PPO_TYPE.toUpperCase()} testing and upload process completed successfully!`);
